import { FIREBASE_REGION, IMAGE_FORMATS, IMAGE_PRESETS, ImageFormat, ImagePresetKey } from '@fazole/config';
...
export const onImageUpload = onObjectFinalized({ region: FIREBASE_REGION }, async (event) => {
  ...
  for (const preset of presetKeys) {
    ...
    for (const format of IMAGE_FORMATS) {
      ...
      const processed = await sharp(buffer).resize({ width, height, fit }).toFormat(format).toBuffer();
